...
YMIN, YMAX = 0.3, 25.0     # pJ/MAC (extends down for the projected analog CIM point)
...
def main():
    ...
    s.append(
        ...
        f'Sky130 INT8 PIM — Pareto Frontier (4 verified + analog CIM projected)</text>'
    )
    ...
    for r in rows:
        ...
        projected = r["measurement"].lower().startswith("projected")
        parallel = int(r["mac_per_cycle"]) >= 256
        col = "#d97706" if projected else ("#2563eb" if parallel else "#059669")
        star = r["design"].lower().startswith("clock")
        rad = 9 if (star or projected) else 6
        if projected:
            # amber diamond, dashed outline -> visually distinct from the measured points
            d = rad + 1
            pts = f"{x:.1f},{y-d:.1f} {x+d:.1f},{y:.1f} {x:.1f},{y+d:.1f} {x-d:.1f},{y:.1f}"
            s.append(
                f'<polygon points="{pts}" fill="{col}" stroke="#7c2d12" '
                f'stroke-width="1.4" stroke-dasharray="3 2"/>'
            )
            tag = f'{r["design"]} ({float(r["pj_per_mac"]):g} pJ INT8 · 5.9 fJ binary · projected)'
        else:
            s.append(
    ...
    s.append(
        ...
        f'blue=parallel · green=near-memory SRAM · ◆amber=charge-domain CIM (projected)</text>'
    )
    s.append(
        f'<text x="{ML+PW-4}" y="{MT+32}" font-size="10.5" fill="#94a3b8" text-anchor="end">'
        f'analog energy from extracted caps (M10) + measured StrongARM; area estimated</text>'
    )
    s.append("</svg>")
